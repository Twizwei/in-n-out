...
class SpaceRegularizer:
    ...
    def ball_holder_loss_lazy(self, new_G, num_of_sampled_latents, w_batch, cam_params, log_name):
        loss = 0.0
        
        z_samples = np.random.randn(num_of_sampled_latents, self.original_G.z_dim)
        w_samples = self.original_G.mapping(torch.from_numpy(z_samples).to(global_config.device), cam_params.repeat(num_of_sampled_latents, 1),
                                            truncation_psi=0.7)   #TODO: probably be problematic
        territory_indicator_ws = [self.get_morphed_w_code(w_code.unsqueeze(0), w_batch) for w_code in w_samples]

        for w_code in territory_indicator_ws:
            if w_code.shape[0] > 1:
                cam_params = cam_params.repeat(w_code.shape[0], 1)
            new_img = new_G.synthesis(w_code, cam_params)['image']
            with torch.no_grad(): 
                old_img = self.original_G.synthesis(w_code, cam_params)['image']
    ...
    def space_regularizer_loss(self, new_G, w_batch, cam_params, log_name):
        ret_val = self.ball_holder_loss_lazy(new_G, hyperparameters.latent_ball_num_of_samples, w_batch, cam_params, log_name)
        return ret_val
