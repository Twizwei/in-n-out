...
from outdomain.depth_alignment import calibrate_disparity
import click
import torch
...
def outdomain_inv(
    ...
    w_plus = latents['w_plus'].to(device).requires_grad_(True)
    ...
    if train_raw:
        ...
        for epoch in range(num_epochs):
            ...
            for itr, data in enumerate(train_dataloader):
                ...
                #     # align depth
                #     depth_map_aligned = calibrate_disparity(depth_map_est, depth_map)
                #     # Charbonnier penalty for output depth `depth_map_aligned` and off-the-shelf depth `depth_map_est`
                #     depth_reg = torch.mean(torch.sqrt(torch.pow(depth_map_aligned.squeeze() - depth_map_est.squeeze(), 2) + 1e-6))
                ...
                # if save_intermediates and data[-1][0] == '/home/yiranx/data/capvideo_simple/11986_16f/frames/frame0203.png':
                    with torch.no_grad():
                        ...
                        output_image = np.concatenate((synth_image_f, synth_image_o, synth_image_full, synth_image_o_sr, synth_image_full_sr, target_image), axis=1)
