        ...
        for epoch in range(num_epochs):
            ...
            for itr, data in enumerate(train_dataloader):
                if video_dataset.depth_maps is not None:
                    ...
                    target_images, masks_tensor, camera_params, w_plus_residual_curr, phi_curr, depth_map_est, _ = data
                else:
        ...
        for epoch in range(num_epochs):
            ...
            for itr, data in enumerate(train_dataloader):
                if video_dataset.depth_maps is not None:
                    ...
                    target_images, masks_tensor, camera_params, w_plus_residual_curr, phi_curr, _, _ = data
                else:
                    # target_images, masks_tensor, camera_params, w_plus_curr, phi_curr, _ = data
                    target_images, masks_tensor, camera_params, w_plus_residual_curr, phi_curr, _ = data
                target_images = target_images.to(device)
    ...
    with torch.no_grad():
        for itr, data in enumerate(eval_dataloader):
            if video_dataset.depth_maps is not None:
                ...
                target_images, masks_tensor, camera_params, w_plus_residual_curr, phi_curr, _, img_path = data
            else:
