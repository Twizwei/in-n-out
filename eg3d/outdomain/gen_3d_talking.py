"""
Generate videos where novel views are generated while person is talking or etc.
"""
import os
import sys
sys.path.append(".")
...
def gen_interp_video(compose_triplanes, phi, mp4: str, seeds, latents_path, w_type='cano', padding=False, shuffle_seed=None, w_frames=60*4, kind='cubic', grid_dims=(1,1), num_keyframes=None, wraps=2, psi=1, truncation_cutoff=14, cfg='FFHQ', image_mode='image', gen_shapes=False, device=torch.device('cuda'), **video_kwargs):
    grid_w = grid_dims[0]
    grid_h = grid_dims[1]
    ...
    ws = latents['w_plus'].to(device)
    # if isinstance(latents, dict):
    #     ws = latents['w_plus'][f_idx:f_idx+1].to(device)
    # elif isinstance(latents, torch.Tensor):
    #     ws = latents[f_idx:f_idx+1].to(device)
    # phi_curr = phi[f_idx:f_idx+1].to(device)
    # _ = G.synthesis(ws[:1], c[:1]) # warm up
    # ws = ws.reshape(grid_h, grid_w, num_keyframes, *ws.shape[1:])

    num_frames = ws.shape[0]
    # Interpolation.
    grid = []
        ...
        for frame_idx in tqdm(range(num_frames)):
            ...
            for yi in range(grid_h):
                for xi in range(grid_w):
                    ...
                    entangle = 'camera'
                    phi_curr = phi[frame_idx:frame_idx+1].to(device)
            ...
            if not padding:
                ...
                video_out.append_data(layout_grid(torch.stack(imgs), grid_w=grid_w, grid_h=grid_h, float_to_uint8=False))
    video_out.close()
...
def generate_images(
    ...
    if interpolate:
        if w_type == 'cano':
            ...
            output = os.path.join(outdir, 'rgb_interpolation_spatiotemporal.mp4')
        gen_interp_video(compose_triplanes, phi, mp4=output, latents_path=latents_path, padding=padding, w_type=w_type, bitrate='10M', grid_dims=grid, num_keyframes=num_keyframes, w_frames=w_frames, seeds=seeds, shuffle_seed=shuffle_seed, psi=truncation_psi, truncation_cutoff=truncation_cutoff, cfg=cfg, image_mode=image_mode, gen_shapes=shapes)
