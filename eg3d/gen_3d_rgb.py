...
Example:
python gen_3d_rgb.py --outdir=./out/optim_multi_dynamic/mask1_full_1.0lpips_1.0l2_lr1e-3_deltanorm_1e-3_0.7res_nomask_prepose_noUpdatePose/shape --trunc=1.0 --seeds=0 --grid=1x1 --network=pretrained_models/ffhqrebalanced512-128.pkl --latents_path ./out/optim_multi_dynamic/mask1_full_1.0lpips_1.0l2_lr1e-3_deltanorm_1e-3_0.7res_nomask_prepose_noUpdatePose/latents.pt --w_type='cano' --shapes=True 
"""
...
def gen_interp_video(G, mp4: str, seeds, latents_path, f_idx, w_type='cano', shuffle_seed=None, w_frames=60*4, kind='cubic', grid_dims=(1,1), num_keyframes=None, wraps=2, psi=1, truncation_cutoff=14, cfg='FFHQ', image_mode='image', gen_shapes=False, device=torch.device('cuda'), **video_kwargs):
    grid_w = grid_dims[0]
    grid_h = grid_dims[1]
    ...
    if isinstance(latents, dict):
        if w_type == 'cano':
            ws = latents['w_plus_cano'].detach().to(device)
        elif w_type == 'w_plus':
...
def generate_images(
    ...
    if network_pkl.endswith("pkl"):
        ...
        if isinstance(network_pkl, dict):
