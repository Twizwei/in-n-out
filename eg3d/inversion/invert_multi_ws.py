...
def project(
    ...
    if random_init:
        ...
        ws_plus_cano_opt = init_vars['w_plus'].clone().detach().cpu()
        ws_plus_cano_opt = ws_plus_cano_opt.clone().to(device).requires_grad_(True)  # kinda not efficient
        ws_plus_residual_opt = torch.zeros(cam_params_opt.shape[0], 14, 512).to(device).requires_grad_(True)
    else:
    ...
    for epoch in range(num_epochs):
        ...
        for itr, data in enumerate(train_dataloader):
            ...
            # synth_images = G.synthesis(w_plus_opt_curr, camera_params_opt_curr)['image']
