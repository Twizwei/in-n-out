...
class ImportanceRenderer(torch.nn.Module):
    ...
    def run_model_extend(self, phi, planes, decoder, sample_coordinates, sample_directions, options):
        sampled_features = sample_from_planes(self.plane_axes, planes, sample_coordinates, padding_mode='zeros', box_warp=options['box_warp'])
        out = decoder.forward_extend(sampled_features, phi)
        if options.get('density_noise', 0) > 0:
            out['sigma'] += torch.randn_like(out['sigma']) * options['density_noise']
        return out
    ...
    def unify_samples(self, depths1, colors1, densities1, depths2, colors2, densities2, return_indices=False):
