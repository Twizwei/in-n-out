...
class TriPlaneGenerator(torch.nn.Module):
    ...
    def synthesis(self, ws, c, neural_rendering_resolution=None, update_emas=False, cache_backbone=False, use_cached_backbone=False, **synthesis_kwargs):
        ...
        if use_cached_backbone and self._last_planes is not None:
            ...
            planes = self.backbone.synthesis(ws, update_emas=update_emas, **synthesis_kwargs)  # [N, 96, 256, 256]
        if cache_backbone:
...
class OSGDecoder(torch.nn.Module):
    ...
    def forward_extend(self, sampled_features, phi):
        """
        Args:
        sampled_features: Triplane features
        phi: framewise latent code [1, C]
        """
        # Aggregate features
        sampled_features = sampled_features.mean(1)
        x = sampled_features
        ...
        N, M, C = x.shape
        x = x.view(N*M, C)  # [N*M, C]
        x = torch.cat([x, phi.expand(N*M, phi.shape[-1])], dim=1)

        x = self.net(x)
        x = x.view(N, M, -1)  # [..., density_dim + color_dim + blending_dim]
        # rgb_blending = torch.sigmoid(x[..., 1:])*(1 + 2*0.001) - 0.001 # Uses sigmoid clamping from MipNeRF

        rgb = torch.sigmoid(x[..., 1:-1])*(1 + 2*0.001) - 0.001 # Uses sigmoid clamping from MipNeRF
        blending = torch.sigmoid(x[..., -1]).unsqueeze(-1)
        sigma = x[..., 0:1]

        # return {'rgb': rgb_blending[..., :-1], 'sigma': sigma, 'blending': rgb_blending[..., -2:-1]}
        return {'rgb': rgb, 'sigma': sigma, 'blending': blending}
