...
def generate_images(
    ...
    for seed_idx, seed in enumerate(seeds):
        ...
        for angle_y, angle_p in [(0, angle_p)]:
            ...
            w_cano = ws
            w_plus = ws 
            w_residual = torch.zeros_like(ws)
            latents = {'w_plus_cano': w_cano.cpu(), 'w_plus': w_plus.cpu(), 'w_plus_residual': w_residual.cpu()}
            cam_intrinsics = intrinsics
            # cam_params = cam2world_pose.reshape(-1, 16)
            cam_params_ = {'cam_intrinsics': cam_intrinsics.cpu(), 'cam_params': cam2world_pose.cpu().reshape(-1, 16)}
            init_vars = {'w': w_plus.cpu(), 'w_plus': w_plus.cpu(), 'camera_params':camera_params.cpu()}
            ############
            ...
            ############
            torch.save(init_vars, os.path.join(outdir, 'init_vars.pt'))
            torch.save(latents, os.path.join(outdir, 'latents.pt'))
            torch.save(cam_params_, os.path.join(outdir, 'cam_params.pt'))
            # save camera parameters into a json file
            import pdb; pdb.set_trace()
            dataset_json = {"labels": [f'frame{seed:04d}.png', [float(param) for param in camera_params.cpu().numpy()[0]]]}
            with open(os.path.join('/fs/nexus-scratch/yiranx/video3dgan/video3dgan/cats-source/aligned', str(seed), 'dataset.json'), 'w') as outfile:
        ...
        outdir_source = '/fs/nexus-scratch/yiranx/video3dgan/video3dgan/cats-source/aligned'
        os.makedirs(os.path.join(outdir_source, str(seed), 'frames'), exist_ok=True)
        PIL.Image.fromarray(img[0].cpu().numpy(), 'RGB').save(f'{outdir_source}/{seed}/frames/frame{seed:04d}.png')
