...
from CLIPStyle.criteria import clip_loss
from CLIPStyle.mapper.mapper_criteria import id_loss
from CLIPStyle.mapper.datasets.latents_dataset import LatentsDataset
from CLIPStyle.mapper.styleclip_mapper import StyleCLIPMapper
from CLIPStyle.mapper.training_mapper.ranger import Ranger
from CLIPStyle.mapper.training_mapper import train_utils
import pdb

class Coach:
	def __init__(self, opts, G, src_text="A person."):
		self.opts = opts
		self.text_src = src_text
		self.global_step = 0
		self.factor_step = self.opts.factor_step

		self.device = 'cuda:0'
		self.opts.device = self.device
		
		# Initialize network
		self.net = StyleCLIPMapper(self.opts, G).to(self.device)

		# Initialize loss
		if self.opts.id_lambda > 0:
			self.id_loss = id_loss.IDLoss(self.opts).to(self.device).eval()
		if self.opts.clip_lambda > 0:
			self.clip_loss = clip_loss.CLIPLoss(device=self.device)
	...
	def train(self):
		...
		intrinsics = torch.tensor([[4.4652, 0.0000, 0.5000],
									...
									[0.0000, 0.0000, 1.0000]], device=self.device).reshape(-1, 9).repeat(self.train_dataloader.batch_size, 1)
		...
		while self.global_step < self.opts.max_steps:
			for batch_idx, batch in enumerate(self.train_dataloader):
				...
				w = batch
				w = w.to(self.device)
				# sample camera angles and get camera parameters
				sample_yaw = (yaw_range * torch.randn(w.shape[0], 1)).to(self.device)
				sample_pitch = (pitch_range * torch.randn(w.shape[0], 1)).to(self.device)
				cam_pivot = torch.tensor(self.net.decoder.rendering_kwargs.get('avg_camera_pivot', [0, 0, 0]), device=self.device)
				...
				cam2world_pose = LookAtPoseSampler.sample(torch.pi/2 + sample_yaw, torch.pi/2 + sample_pitch, cam_pivot, batch_size=w.shape[0], radius=cam_radius, device=self.device)
				conditioning_cam2world_pose = LookAtPoseSampler.sample(torch.pi/2, torch.pi/2, cam_pivot, batch_size=w.shape[0], radius=cam_radius, device=self.device)
				camera_params = torch.cat([cam2world_pose.reshape(-1, 16), intrinsics.reshape(-1, 9)], 1)

				with torch.no_grad():
					# x, _ = self.net.decoder([w], input_is_latent=True, randomize_noise=False, truncation=1)
					x = self.net.decoder.synthesis(w, camera_params)['image']
				w_hat = w + self.factor_step * self.net.mapper(w)
				# x_hat, w_hat = self.net.decoder([w_hat], input_is_latent=True, return_latents=True, randomize_noise=False, truncation=1)
				x_hat = self.net.decoder.synthesis(w_hat, camera_params)['image']
				...
				if self.global_step % self.opts.val_interval == 0 or self.global_step == self.opts.max_steps:
					val_loss_dict = self.validate(camera_params)
	...
	def validate(self, camera_params):
		self.net.eval()
		agg_loss_dict = []
		for batch_idx, batch in enumerate(self.test_dataloader):
			...
			with torch.no_grad():
				...
				x = self.net.decoder.synthesis(w, camera_params[:self.test_dataloader.batch_size])['image']
				w_hat = w + self.factor_step * self.net.mapper(w)
				# x_hat, _ = self.net.decoder([w_hat], input_is_latent=True, randomize_noise=True, truncation=1)
				x_hat = self.net.decoder.synthesis(w_hat, camera_params[:self.test_dataloader.batch_size])['image']
		...
		if self.opts.latents_train_path:
			...
			for b in range(self.opts.train_dataset_size // self.opts.batch_size):
				with torch.no_grad():
					...
					_, train_latents_b = self.net.decoder.mapping(train_latents_z[b: b + self.opts.batch_size], torch.zeros_like(c), truncation_psi=0.7)
	...
	def calc_loss(self, w, x, w_hat, x_hat):
		...
		if self.opts.clip_lambda > 0:
			loss_clip = self.clip_loss.global_clip_loss(x_hat, self.text_inputs).mean()
		...
		if self.opts.norm_lambda > 0:
			loss_norm = self.clip_loss.norm_loss(x, x_hat)
			loss_dict['loss_norm'] = float(loss_norm)
			loss += loss_norm * self.opts.norm_lambda
		if self.opts.lambda_direction > 0:
			loss_direction = self.clip_loss.clip_directional_loss(x, self.text_src, x_hat, self.text_inputs)
			loss_dict['loss_direction'] = float(loss_direction)
			loss += loss_direction * self.opts.lambda_direction
