...
from CLIPStyle.utils.text_templates import imagenet_templates, part_templates, imagenet_templates_small
...
class CLIPLoss(torch.nn.Module):
    def __init__(self, device, lambda_direction=1., lambda_patch=0., lambda_global=0., lambda_manifold=0., lambda_texture=0., patch_loss_type='mae', direction_loss_type='cosine', clip_model='ViT-B/32'):
        ...
        # self.upsample = torch.nn.Upsample(scale_factor=6)
    ...
    def norm_loss(self, image_pre, image_post):
        norm_pre = self.encode_images(image_pre).norm(dim=-1)
        norm_post = self.encode_images(image_post).norm(dim=-1)

        return self.texture_loss(norm_pre, norm_post)
