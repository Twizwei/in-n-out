...
"""Generate frames for a single identity.
Example:
python gen_syn_data.py --outdir=/home/yiranx/data/eg3d_syn/person_00 --trunc=0.7 --seeds=0 --grid=1x1 --network=pretrained_models/ffhqrebalanced512-128.pkl
"""
...
def gen_interp_video(G, mp4: str, seeds, shuffle_seed=None, w_frames=60*4, kind='cubic', grid_dims=(1,1), num_keyframes=None, wraps=2, psi=1, truncation_cutoff=14, cfg='FFHQ', image_mode='image', gen_shapes=False, device=torch.device('cuda'), **video_kwargs):
    ...
    output_dir = os.path.dirname(mp4)
    torch.save({'w_plus': ws.detach().clone().cpu()}, os.path.join(output_dir, 'latent_w.pt'))
    ...
    for frame_idx in tqdm(range(num_keyframes * w_frames)):
        ...
        for yi in range(grid_h):
            for xi in range(grid_w):
                pitch_range = 0.25
                yaw_range = 0.35
                yaw_curr = yaw_range * np.sin(2 * 3.14 * frame_idx / (num_keyframes * w_frames))
                pitch_curr = -0.05 + pitch_range * np.cos(2 * 3.14 * frame_idx / (num_keyframes * w_frames))
                ...
                cam_params['yaw'].append(yaw_curr)
                cam_params['pitch'].append(pitch_curr)
                cam_params['proj_mat'].append(c.clone().cpu())
                interp = grid[yi][xi]
                ...
                img_np = (img * 127.5 + 128).clamp(0, 255).to(torch.uint8).permute(1,2,0).cpu().numpy()
                imageio.imwrite(os.path.join(outdir_frames, 'frame{0:05d}.png'.format(frame_idx)), img_np)
                imgs.append(img)
    ...
    cam_params['yaw'] = torch.from_numpy(np.array(cam_params['yaw']))
    cam_params['pitch'] = torch.from_numpy(np.array(cam_params['pitch']))
    cam_params['proj_mat'] = torch.cat(cam_params['proj_mat'])
    torch.save(cam_params, os.path.join(output_dir, 'cam_params.pt'))
